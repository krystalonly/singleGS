...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        if "deformation" in training_args.train_l:
            train_l.append({'params': list(self._deformation.get_mlp_parameters()),
                            'lr': training_args.deformation_lr_init * self.spatial_lr_scale, "name": "deformation"})
        if "grid" in training_args.train_l:
            train_l.append({'params': list(self._deformation.get_grid_parameters()),
    ...
    def load_ply(self, path):
        print(f"tring to load ply file from: {path}")
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # stds: [N, 2] => [N, 3]
        if stds.shape[1] == 2:
            zeros = torch.zeros(stds.shape[0], 1, device=stds.device, dtype=stds.dtype)
            stds = torch.cat([stds, zeros], dim=1)  # z方向方差设为0或很小
    ...
    def grow(self, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        _, low_density_points, new_points, low_density_index = addpoint(point_cloud[downsampled_point_index],
