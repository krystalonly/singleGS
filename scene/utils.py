...
def get_jaw_features(features, att_mode, index):
    # features: shape (8000,)
    ...
    if att_mode == 0:
        ...
        # 选择以 index 为中心的窗口，前8帧，后0帧，进行零填充
        left = index - 8
        pad_left = 0
        if left < 0:
            pad_left = -left
            left = 0
        jaw_feats = features[left:index]
        if pad_left > 0:
            jaw_feats = np.concatenate([np.zeros(pad_left), jaw_feats], axis=0)
        return jaw_feats

    elif att_mode == 2:
        # 选择以 index 为中心的窗口，前4帧，后4帧，进行零填充
        left = index - 4
        right = index + 4
        pad_left = 0
        pad_right = 0
        if left < 0:
            ...
            left = 0
        if right >= features.shape[0]:
            pad_right = right - features.shape[0] + 1
            right = features.shape[0] - 1
        jaw_feats = features[left:right + 1]  # Include right frame
        print("pad_left:", pad_left)
        print("np.zeros(pad_left).shape:", np.zeros(pad_left).shape)
        print("jaw_feats.shape:", jaw_feats.shape)
        if pad_left > 0:
            jaw_feats = np.concatenate([np.zeros(pad_left), jaw_feats], axis=0)
        if pad_right > 0:
            jaw_feats = np.concatenate([jaw_feats, np.zeros(pad_right)], axis=0)
        return jaw_feats
