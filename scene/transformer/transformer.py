...
class Spatial_Audio_Attention_Layer(nn.Module):
    def __init__(self, args):
        ...
        self.time_linear = nn.Linear(64, self.args.d_model)

    def forward(self, x, enc_source, time):
        _x = x

        x = x + self.time_linear(time.expand(-1, x.shape[1], -1))
...
class Attention(nn.Module):
    ...
    def forward(self, x, enc_source):
        # 生成查询、键、和值
        Q = self.query_linear(x)  # 输入特征生成查询
        K = self.key_linear(enc_source)  # 编码源特征生成键
        V = self.value_linear(enc_source)  # 编码源特征生成值
...
class AttentionModule(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.attention = Attention(feature_dim=args.d_model)
        self.time_linear = nn.Linear(64, args.d_model)
    ...
    def forward(self, x, enc_source, time):
        ...
        """
        x = x + self.time_linear(time.expand(-1, x.shape[1], -1))
        x, attention_weights = self.attention(x, enc_source)
        return x, attention_weights
