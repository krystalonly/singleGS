...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
    ...
    if opt.dataloader:
        ...
        if opt.custom_sampler is not None:
            ...
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size, sampler=sampler, num_workers=0,
                                                collate_fn=list, drop_last=True)
            random_loader = False
        else:
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size, shuffle=True, num_workers=0,
                                                collate_fn=list, drop_last=True)
            random_loader = True
    ...
    for iteration in range(first_iter, final_iter + 1):  # 여기부터 iteration 시작
        ...
        random_color = False
        output = render_from_batch(viewpoint_cams, gaussians, pipe, random_color, stage=stage,
        ...
        # render_alpha_tensor = output["alpha_tensor"]
        # render_normal_tensor = output["normal_tensor"]
        # render_dist_tensor = output["dist_tensor"]
        # surf_depth_tensor = output["surf_depth_tensor"]
        # surf_normal_tensor = output["surf_normal_tensor"]
        # normal_error = (1 - (render_normal_tensor * surf_normal_tensor).sum(dim=0))[None]
        # normal_loss = lambda_normal * (normal_error).mean()
        # dist_loss = lambda_dist * (render_dist_tensor).mean()

        radii = output["radii"]
        viewspace_point_tensor_list = output["viewspace_point_tensor_list"]
        viewspace_point_tensor = viewspace_point_tensor_list[0]
        is_silent_tensor = output["is_silent_tensor"]

        silent_img_loss = ((image_tensor - gt_image_tensor) ** 2).mean(dim=[1, 2, 3])
        silent_img_loss = (silent_img_loss * is_silent_tensor.squeeze(-1)).mean()
        ...
        loss = 0.8 * Ll1 + 0.01 * perceptual_loss + 0.2 * (1.0 - ssim_loss) + silent_img_loss
        ...
        with torch.no_grad():
            ...
            if use_wandb:
                if iteration % 10 == 0:
                    log_dict = {"Ll1": Ll1.item() * 0.8, "psnr": psnr_,
                                "perceptual_loss": perceptual_loss.item() * 0.01,
                                "ssim_loss": 0.2 * (1 - ssim_loss.item()), "loss": loss.item(),
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,
             debug_from, expname, use_wandb):
    # first_iter = 0
    tb_writer = prepare_output_and_logger(expname)
    if use_wandb:
        wandb.init(project="TalkingGaussians", name=expname)

    # path = os.path.join(args.source_path, "gt_imgs")
    # img_files = os.listdir(path)
    # random_idx = random.randint(0, len(img_files)-1)
    # random_img = os.path.join(path, img_files[random_idx])
    # hyper.landmarks = extract_landmarks(opt.batch_size,path)
...
def evaluate_video(video_dir, GT_video_dir, short_configs='Obama2'):
    os.system(
        f"python eval/compare_video.py --video_dir {video_dir} --GT_video_dir {GT_video_dir} --store_result --short_configs {short_configs} --face_crop ")
