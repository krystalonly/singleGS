...
def render_set(model_path, name, iteration, scene, gaussians, pipeline, audio_dir, batch_size):
    ...
    viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size, shuffle=False, num_workers=0,
                                        collate_fn=list)
...
def render_sets(dataset: ModelParams, hyperparam, iteration: int, pipeline: PipelineParams, args):
    ...
    with torch.no_grad():
        ...
        if args.custom_aud != '':
            ...
            render_set(dataset.model_path, "custom", scene.loaded_iter, scene.getCustomCameras(), gaussians, pipeline,
                       audio_dir, batch_size)
        ...
        if not skip_test:
            ...
            render_set(dataset.model_path, "test", iteration, scene.getTestCameras(), gaussians, pipeline, audio_dir,
                       batch_size)
